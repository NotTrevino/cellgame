...
function updateInstructions() {
    ...
    if (gameMode === 1) {
        instr.innerHTML = "Use WASD or arrow keys to move &mdash; press any key to start the timer!<br>"
...
function getKeyAndMove(e) {
    // 1P mode: start stopwatch on first keypress
    ...
    switch (key_code) {
        case 65: moveredLeft();  break; // A
        case 87: moveredUp();    break; // W
        case 68: moveredRight(); break; // D
        case 83: moveredDown();  break; // S

        case 37: // left arrow
            if (gameMode === 2) moveblueLeft();  else moveredLeft();  break;
        case 38: // up arrow
            if (gameMode === 2) moveblueUp();    else moveredUp();    break;
        case 39: // right arrow
            if (gameMode === 2) moveblueRight(); else moveredRight(); break;
        case 40: // down arrow
            if (gameMode === 2) moveblueDown();  else moveredDown();  break;
    }
}
...
function updateBalls() {
    ...
    // ── Food collision ──────────────────────────────────────────────────────
    var rL = parseInt(redball.style.left),  rR2 = rL + getRadius(redball)  * 2;
    var rT = parseInt(redball.style.top),   rB  = rT + getRadius(redball)  * 2;
    var bL = parseInt(blueball.style.left), bR2 = bL + getRadius(blueball) * 2;
    ...
    for (var i = foodElements.length - 1; i >= 0; i--) {
        ...
        if (rR2 >= fR && rL <= fL && rB >= fB2 && rT <= fT2) {
            redballgrowth();
            food.remove();
            continue;
        }
        if (bR2 >= fR && bL <= fL && bB >= fB2 && bT <= fT2) {
            blueballgrowth();
            food.remove();
        }
    }
    ...
    rL = parseInt(redball.style.left);  rR2 = rL + getRadius(redball)  * 2;
    rT = parseInt(redball.style.top);   rB  = rT + getRadius(redball)  * 2;
    bL = parseInt(blueball.style.left); bR2 = bL + getRadius(blueball) * 2;
    bT = parseInt(blueball.style.top);  bB  = bT + getRadius(blueball) * 2;

    if (rR2 >= bR2 && rL <= bL && rB >= bB && rT <= bT && getRadius(redball) > getRadius(blueball)) {
        redballgrowth();
        blueball.remove();
    }
    if (bR2 >= rR2 && bL <= rL && bB >= rB && bT <= rT && getRadius(blueball) > getRadius(redball)) {
        blueballgrowth();
        redball.remove();
    }
